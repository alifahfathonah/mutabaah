...
require "App.php";
class Mentoring_mutabaah extends App
{
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $user_mutabaah) {
            ...
            $data[] =[
                ...
                '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_mutabaah(' 
                . "'" . $user_mutabaah->idmutabaah . "'" . ')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
                <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="del_mutabaah(' 
                . "'" . $user_mutabaah->idmutabaah . "'," ."'". $user_mutabaah->kode_kelompok."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>'
            ];
        }
    ...
    public function input_mutabaah(){
        $sess = $this->session->userdata('ses_mentoring');
        if($this->ModelMentoring->hitung("mutabaah",[
                "pertemuan_idpertemuan" => $this->input->post('pertemuan_idpertemuan'),
                "user_iduser" =>  $sess['id']
        ])==0){
            $data = $this->input->post();
            $data["user_iduser"] = $sess['id'];
            $this->ModelMentoring->create("mutabaah",$data);
            echo json_encode(["status" => TRUE, $data]);
        }else{
            echo json_encode(["status" => FALSE,"message"=>"Pertemuan sudah ada"]);
        }
    }
    ...
    public function edit_mutabaah($id){
        ...
        if($this->ModelMentoring->hitung("mutabaah",[
            ...
            $data = $this->input->post();
            $this->ModelMentoring->update(["idmutabaah"=>$id],$data);
            echo json_encode(["status" => TRUE, $data]);
        }else{
    ...
    public function delete_mutabaah($id){
        if($this->ModelMentoring->delete('mutabaah',["idmutabaah"=>$id])){
    ...
    public function get_mutabaah_by_id($id){
        $sess = $this->session->userdata('ses_mentoring');
        $mutabaah = $this->ModelMentoring->select("*","mentoring,kelompok",[
                                "user_iduser" => $sess['id']
                            ]);
